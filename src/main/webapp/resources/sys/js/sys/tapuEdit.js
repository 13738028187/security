...
var vm = new Vue({
	...
	methods: {
       ...
       save :function(){
    	  $.ajax({
	            type: "POST",
	            url: "../topoManage/saveTopo?table=editTopo",
       ...
       upload :function(){
    	   $.ajax({
	            type: "POST",
	            url: "../topoManage/saveTopo?table=topo",
	            data:JSON.stringify(oldtopo),
	            success: function(r){
	               if(r.code==0){
	            	   alert("保存成功");
    ...
    //新增节点
      insertNode:function(){    	  
    	  ...
    	  for(i=0;i<nodes.length;i++){
    		  if(nodes[i].ip==vm.link.source){
    			  flag=1;    			  
    		  }
    		  if(vm.link.target.trim()!=""&&nodes[i].ip==vm.link.target){
    			  targetNode=nodes[i];
    		  }
    	  }
    	  if(flag==1){
    	  ...
    	  if(flag==0){
    		//复制一个节点   		  
    		  var copyNode=JSON.parse(JSON.stringify(nodes[0]));    		  
    		  copyNode.ip=vm.link.source;
    	  ...
    	  if(vm.link.target.trim()!=""){
    		  ...
    		  //复制一个设备
    		  var copyLink=JSON.parse(JSON.stringify(oldtopo.links[0]));
    		  //记得更改节点linkCount
       ...
       removeNode:function(){
    	vm.nodes=[];
    	var nodes=document.getElementsByName("nodeIp")
       	for(i=0;i<nodes.length;i++){
    	...
    	//删除第一个空字符串
       	...
       	for(i=0;i<vm.nodes.length;i++){
       		...
       		for(j=0;j<oldtopo.nodes.length;j++){
       			//确保要删除的节点最多只有一条设备
       			if(oldtopo.nodes[j].ip==vm.nodes[i]){
       				flag=1;
       				if(oldtopo.nodes[j].linkCount>1){
       					...
       					for(var k=0;k<oldtopo.links.length;k++){
       						if(oldtopo.links[k].source.ip==oldtopo.nodes[j].ip){
    ...
    //新增设备
      insertLink:function(){
    	  ...
    	  var modleLink={
    		  ...
    		  for(var i=0;i<oldtopo.nodes.length;i++){
        		  if(oldtopo.nodes[i].ip==links[0].value){
        			  var sourceNode=oldtopo.nodes[i];
        		  }
        		  if(oldtopo.nodes[i].ip==links[1].value){
      ...
      removeLink:function(){
    	  ...
    	  for(var i=0;i<oldtopo.links.length;i++){
    		  ...
    		  if(oldtopo.links[i].source.ip==links[0].value){
    			 if(oldtopo.links[i].target.ip==links[1].value){
    				 flag=1;
    		  ...
    		  }else if(oldtopo.links[i].source.ip==links[1].value){
    			  if(oldtopo.links[i].target.ip==links[0].value){
    				 flag=1;
     				 oldtopo.links.splice(i,1);
     				 vm.redraw();
     				 alert("删除链路成功");
...
/*var a=setInterval(function() {
	...
	for(var i=0;i<nodes.length;i++){
		if(nodes[i].__data__.id=="172.16.146.6"){
			node=nodes[i];		
		}		
	}
	if(node!=null){
